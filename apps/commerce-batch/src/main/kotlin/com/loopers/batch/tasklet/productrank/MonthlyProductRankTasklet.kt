...
@Component
@StepScope
class MonthlyProductRankTasklet(
    ...
    override fun execute(contribution: StepContribution, chunkContext: ChunkContext): RepeatStatus? {
        val top100 = productMetricsMonthlyRepository.findTopByAggregateDateOrderByScoreDesc(aggregateDate, TOP_N_SIZE)
        ...
        // 기존 데이터 삭제
        mvProductRankMonthlyRepository.deleteByAggregateDate(aggregateDate)

        mvProductRankMonthlyRepository.saveAll(ranked)
        log.info("aggregateDate={}에 대해 mv_product_rank_monthly에 {}개의 랭킹 데이터 저장 완료", aggregateDate, ranked.size)
